# calculate z scores for every 5 sec from 5 sec to 55 sec for training and from 5 sec to 45 sec for tests
dF.all.z.tone <- function(folder){
    library(tidyverse)
    ...
    # save file
    write_csv(temp, file.path(folder, "z tone training.csv"))
    ...
    for(i in 2:length(folders)){
        ...
        # save file
        write_csv(temp, file.path(folder, paste0("z tone test", i-1, ".csv")))
    }
