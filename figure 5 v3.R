...
freezing.mean <- freezing %>%
    group_by(Phase) %>%
    summarise(Freezing = mean(Freezing), N = n(), SE = sd(Freezing)/sqrt(N))
...
correlation1 <- filter(correlation, Phase != "Test 3")

# calculate correlation
model <- lm(Freezing ~ Active, data = correlation)
summary(model) # R-squared: 0.69, p: 0.04

model1 <- lm(Freezing ~ Active, data = correlation1)
summary(model1) # R-sqared: 0.98, p: 0.001

# figure 5b correlation plot
f5b <- ggplot(correlation, aes(x = Active, y = Freezing))+
    geom_point(aes(color = Type), size = 1.5)+
    geom_errorbar(aes(ymin = Freezing - SE, ymax = Freezing + SE, color = Type))+
    scale_color_lancet(guide = FALSE)+
    geom_text(aes(label = Phase, x = Active - 1, y = Freezing + 3, color = Type), size = 4, hjust = 1)+
    annotate("text", label = "paste(\"with Test 3: \", r^2 == 0.69, \", \", p == 0.04)", 
             parse = TRUE, x = 9, y = 80, size = 3, color = "orange")+
    annotate("text", label = "paste(\"without Test 3: \", r^2 == 0.98, \", \", p == 0.001)", 
             parse = TRUE, x = 12, y = 73, size = 3, color = "purple")+
    xlim(-10, 105)+
    stat_smooth(method = lm, color = "orange")+
    stat_smooth(data = correlation1, aes(x = Active, y = Freezing), 
