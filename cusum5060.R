cusum5060 <- function(files, d, d_cusum){
    ...
    for(i in 1:(ncol(fileone)-2)){
        ...
        # subset from 50 to 60 sec, calculate row means
        dat_temp <- filter(d, time>=50, time<=60)
        dat_temp <- mutate(dat_temp, mean=rowMeans(dat_temp[,-1], na.rm=TRUE))
        
        # calculate baseline and variance of baseline
        baseline <- mean(dat_temp$mean[dat_temp$time >= 50 & dat_temp$time <= 55])
        ...
        bl[i] <- sd(dat_temp$cusum[dat_temp$time >= 50 & dat_temp$time <= 55])
        avg[i] <- mean(dat_temp$cusum[dat_temp$time >= 50 & dat_temp$time <= 55])
