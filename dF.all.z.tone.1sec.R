...
dF.all.z.tone.1sec <- function(folder){
    library(tidyverse)
    source('dF.training.R')
    source('dF.test.R')
    source("z.dF.tone.1sec.R")
    ...
    temp <- z.dF.tone.1sec(temp)
    
    # save file
    write_csv(temp, file.path(folder, "z tone 1sec training.csv"))
    ...
    for(i in 2:length(folders)){
        ...
        # save file
        write_csv(temp, file.path(folder, paste0("z tone 1sec test", i-1, ".csv")))
    }
