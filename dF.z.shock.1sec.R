# calculate z scores for 1 sec before and after shock for training
dF.z.shock.1sec <- function(folder){
    library(tidyverse)
    source('dF.shock.R')
    source("z.dF.shock.1sec.R")
    ...
    temp <- z.dF.shock.1sec(temp)
    
    # save file
    write_csv(temp, file.path(folder, "z shock 1sec.csv"))
}