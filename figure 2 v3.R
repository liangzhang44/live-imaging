...
early.tone.mean <- mean(early2.mean$dF[early2.mean$time < 10 & early2.mean$time > 5])
early.tone.sd <- sd(early2.mean$dF[early2.mean$time < 10 & early2.mean$time > 5])
early2.tone <- early2 %>%
    filter(time > 5, time < 55) %>%
    mutate(z = (dF - early.tone.mean)/early.tone.sd, phase = "Sessions 1&2")
...
late2.mean <- late2 %>%
    group_by(time) %>%
    summarise(dF = mean(dF))
late.tone.mean <- mean(late2.mean$dF[late2.mean$time < 10 & late2.mean$time > 5])
late.tone.sd <- sd(late2.mean$dF[late2.mean$time < 10 & late2.mean$time > 5])
late2.tone <- late2 %>%
    filter(time > 5, time < 55) %>%
    mutate(z = (dF - late.tone.mean)/late.tone.sd, phase = "Sessions 6&7")
...
# figure 2e
early.shock.mean <- mean(early2.mean$dF[early2.mean$time < 55 & early2.mean$time > 50])
early.shock.sd <- sd(early2.mean$dF[early2.mean$time < 55 & early2.mean$time > 50])
early2.shock <- early2 %>%
    filter(time > 50, time < 65) %>%
    mutate(z = (dF - early.shock.mean)/early.shock.sd, phase = "Sessions 1&2")

late.shock.mean <- mean(late2.mean$dF[late2.mean$time < 55 & late2.mean$time > 50])
late.shock.sd <- sd(late2.mean$dF[late2.mean$time < 55 & late2.mean$time > 50])
late2.shock <- late2 %>%
    filter(time > 50, time < 65) %>%
    mutate(z = (dF - late.shock.mean)/late.shock.sd, phase = "Sessions 6&7")
...
f2e <- ggline(training.shock, x = "time", y = "z", add = "mean_se", color = "phase", 
    ...
    annotate("rect", xmin = 50, xmax = 65, ymin = -8, ymax = -7, color = "black", fill = "green", alpha = 0.4)+
    annotate("rect", xmin = 55, xmax = 56, ymin = -8, ymax = -7, alpha = 0.5, fill = "red")+
    annotate("text", x = 55.5, y = -7.5, label = "Shock", color = "red", size = 4)+
    annotate("text", x = 52.5, y = -7.5, label = "Trace", size = 4)+
    geom_vline(xintercept = 55, lty = 2, color = "red")+
