...
auc.aver <- data.frame(cell = colnames(late2)[-1], test1 = NA,
                       test2 = NA, test3 = NA, test4 = NA)
for (i in 1:4) {
    assign("temp", get(paste0("test", i)))
    for (j in 2:ncol(late2)) {
        auc.aver[j-1, i+1] <- auROC(temp[,j], late2[,j])
    }
}
auc.aver$mouse <- 1
...
auc.all <- data.frame()
for (i in 1:4) {
    auc <- data.frame(cell = colnames(late2)[-1], trial1 = NA,
                      trial2 = NA, trial3 = NA, trial4 = NA, trial5 = NA)
    for (j in 1:5) {
        assign("temp", get(paste0("test", i, ".trial", j)))
        for (k in 2:ncol(late2)) {
            auc[k-1, j+1] <- auROC(temp[,k], late2[,k])
        }
    }
    auc$mouse <- 1
...
f6a <- ggplot(correlation.aver, aes(x = Freezing, y = Discrimination, color = Phase))+
    ...
    annotate("text", x = -0.8, y = 0.9, size = 5, 
